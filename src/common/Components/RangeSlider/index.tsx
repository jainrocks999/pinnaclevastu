...
import { useThumbFollower, useLowHigh, useWidthLayout, useLabelContainerProps, useSelectedRail } from './Hookes';
...
const Slider: React.FC<SliderProps> = ({
  ...
  }, [highProp, lowProp, inPropsRefPrev]);
  ...
  const lowStyles = useMemo(() => ({ transform: [{ translateX: lowThumbXRef.current }] }), [lowThumbXRef]);
  const highStyles = useMemo(() => (
    disableRange ? null : [styles.highThumbContainer, { transform: [{ translateX: highThumbXRef.current }] }]
  ), [disableRange]);
  ...
  const { panHandlers } = useMemo(() => PanResponder.create({
    ...
    onMoveShouldSetPanResponder: (evt: GestureResponderEvent, gestureState: PanResponderGestureState) => Math.abs(gestureState.dx) > 2 * Math.abs(gestureState.dy),
