          ...
          if (fromScreen) {
            ...
            if (localCartDataList.length > 0) {
              for (const item of localCartDataList) {
                // await AsyncStorage.removeItem('cartItems');
                console.log('post api called ho gai hai bhai cart se...');
                await dispatch(
                  addToCartApi({
                    user_id: userData.user_id,
                    itemId: item.id,
...
;
  ...
  const totalAmount = subtotal !== savings ? (subtotal + Tax) - savings : (subtotal + Tax);
      ...
      {(isLoggedIn ? cartDataList.length > 0 : localCartDataList.length > 0) ? (
        <>
          ...
          <View style={styles.subtotalsavingyview}>
            ...
            <Text style={[styles.subtotaltext,{color:colors?.grey}]}> Shipping charges will be calculated in the next step.</Text>
            <TouchableOpacity
          onPress={() => {
            isLoggedIn
              ? navigation.navigate('AddressList',{item:cartDataList,ammount:totalAmount})
