...
const RemediesProductDetail = ({route}) => {
  const item =route?.params?.data?.id;
    ...
    const checkIfInCart = () => {
      ...
      const cartItem =
        userType === undefined
          ...
          console.log(cartItem, 'sandeep sdmkdsfsd');
      if (cartItem) {
        setIsInCart(true);
      } else {
        setIsInCart(false);
      }
      console.log(cartDataList, 'sandeep online data');
      console.log(Detail, 'sandeep...............');
      setCurrentItemInCart(cartItem);
    };

    checkIfInCart();
  }, [localCartDataList, cartDataList]);
  ...
  // console.log(quantity, ' Qty outer ');

  const handleAddToCart = async prod => {
    const userStatus = await AsyncStorage.getItem('user_data');
    const userData = JSON.parse(userStatus);

    const cartItem = cartDataList.find(item => item.product_id === prod.id);

    if (cartItem) {
      const rowId = cartItem.rowid;
      console.log(rowId);
      handleUpdateCartData(userData.user_id, rowId, quantity, userData.token);
      await dispatch(
        getCartDataApi({
